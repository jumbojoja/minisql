...
uint32_t CatalogMeta::GetSerializedSize() const {
  return sizeof(uint32_t) //CATALOG_METADATA_MAGIC_NUM
      +  sizeof(uint32_t) //meta_page_size
      +  sizeof(uint32_t) //index_meta_page.size()
...
dberr_t CatalogManager::CreateTable(const string &table_name, TableSchema *schema, Txn *txn, TableInfo *&table_info) {
  ...
  auto table_meta = TableMetadata::Create(next_table_id_, table_name, root_page_id, schema);
...
dberr_t CatalogManager::GetTables(vector<TableInfo *> &tables) const {
  for(auto iterator:tables_){
    tables.push_back(iterator.second);
  }
  return DB_SUCCESS;
  // ASSERT(false, "Not Implemented yet");
  //return DB_FAILED;
